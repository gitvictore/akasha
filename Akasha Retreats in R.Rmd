---
title: "🌳 **Aldeia Akasha**</br>Ayahuasca and Nature & Social Connectedness</br><span style='font-size:0.5em;'>1H 2025 Results</span>"
author: "Victor Eduardo and Michèle Barocchi, PhD"
...
This pilot study involved administering **three questionnaires (*Baseline*, *Post-Retreat*, and *One-Month*)** to participants at **four ceremonial ayahuasca retreats** conducted at **Aldeia Akasha in Petrópolis, Rio de Janeiro, Brazil** from **January to April 2025**. Each retreat consisted of **three (3) separate ayahuasca ceremonies** over the course of one week.

Participants were invited to complete the *Baseline* questionnaire electronically (via Qualtrics) at the start of the retreat, **prior to the first ceremony**. Participants provided informed consent by affirmatively responding to the survey consent item at the start of the first questionnaire. This small pilot study did not undergo formal IRB review due to its limited scope and programmatic nature.

The ***Baseline*** **questionnaire** asked participants about **demographics** (age, gender, highest education level, country of origin and residence, primary language spoken, religious affiliation, and prior experience with spiritual or healing practices), **previous ayahuasca experience**, and participants' **main intention** for participating in the ceremony. Due to the **internationally diverse audience**, the "highest education level" question specifically included **examples from various educational systems** (e.g., *Ensino Fundamental*, *Lycée*, *Grundschule*), and the survey was offered in **English and Portuguese**.

Participants completed a section covering **recent** **wellbeing over the prior two weeks** (calmness, irritability, anxiety, depressive symptoms, overall life satisfaction) in the *Baseline* and *One-Month* questionnaires. A separate section covering nature and social connectedness was offered in the *Baseline*, *Post-Retreat*, and *One-Month* questionnaires.

All three of these sections employed a 5-point Likert scale (ranging from "Strongly disagree" to "Strongly agree", or "Not at all important" to "Extremely important"). The ordering of questions within these latter sections was randomized. Upon completion of the *Baseline* questionnaire, each participant was assigned an anonymous Participant ID to facilitate longitudinal data tracking across questionnaires.

Immediately following the retreat, participants received a post-retreat survey, which captured their immediate experiences and perceived effects after the ceremonies. This survey repeated the nature and social connectedness measures from the baseline, allowing comparison of pre- and post-retreat responses. Additional questions were asked about specific post-ceremonial effects (e.g., feelings of peace, connectedness, emotional balance, and insights about personal challenges). Participants also rated the importance of specific aspects of the ceremonial setting, including presence of shamanic figures, music, group rituals, and the physical environment, on a similar 5-point Likert scale. Open-ended items solicited participants to share impactful moments and the primary emotion they experienced post-retreat.

The final *One-Month* questionnaire, conducted one month after the retreat, reassessed participants' recent well-being, nature and social connectedness, and mental health status, using identical questions from the previous questionnaires. This allowed for longitudinal comparisons at baseline, immediately post-retreat, and one month later.

Data from all three questionnaires were linked anonymously using the Participant ID, ensuring confidentiality and enabling consistent longitudinal tracking of participants' experiences and outcomes.

## Results

**Table 1** (below) reports characteristics for the **63 participants** who completed the first *(Baseline)* questionnaire. **Most participants were women (58.7 %)** with an **average age of 42 years** (SD = 9.06, range 24–62). **Educational attainment was high**: 30.2 % held a bachelor’s degree, 25.4 % a master’s, 19.0 % vocational training, 15.9 % upper secondary, 7.9 % doctoral, and 1.6 % primary schooling only. The sample was **internationally diverse; most originated from Brazil (30.2 %)**, United Kingdom (19.0 %), and United States (17.5 %), and **the largest group resided in Brazil (33.3 %)** with substantial representation in the UK (25.4 %) and United States (17.5 %).

Ceremony **intentions were led by spiritual growth** (50.8 %), mental health (19.0 %), and career (14.3 %), with smaller proportions citing physical health (4.8 %), curiosity (4.8 %), or other purposes (6.3 %). Prior ayahuasca experience varied widely: **36.5 % had taken it more than ten times**, 17.5 % once, 11.1 % two–five times, 14.3 % six–ten times, and 20.6 % were first-timers.
...
```{r table2 survey completion, echo=FALSE}
...
cleaned_wb_complete <- cleaned_wb %>%
  filter(!is.na(Response)) %>%
  group_by(pid) %>%
...
### Past Ayahuasca Exploration

```{r ayahuasca frequency, echo=FALSE, fig.width=10, fig.height=10, fig.align='center'}
# 1. Tabulate counts per PastAyahuasca level
freq <- base_clean %>%
  count(PastAyahuasca) %>%
...
ggplot(freq, aes(x = PastAyahuasca, y = n)) +
  ...
  labs(
    ...
    title = "Frequency of Prior Ayahuasca Experience (Baseline)"
  ) +
  theme_minimal(base_size = 14) +
  theme(panel.grid.minor = element_blank())
```

```{r ayahuasca-by-intention-plotly, echo=FALSE, fig.align='center', fig.width=10, fig.height=6}
...
```{r chart: connectedness across timepoints, echo=FALSE, fig.width=16, fig.height=8}
### nat
...
This table summarizes paired comparisons between *Baseline* and both subsqequent questionnaires (*Follow-up* and *One-month*) across 11 questions on **Nature and Social connectedness**.
...
Notably, the **most robust effects** were for “*I feel like I belong*” and “*I feel understood by others*,” both showing highly significant improvements (p \< .001). In contrast, items related to **nature connection** (e.g., awe in nature, spiritual bond with Earth, emotional connection to the natural world) and environmental motivation **showed** **no significant changes**. This pattern suggests the retreat had a **stronger and more immediate impact on social rather than nature-based aspects of connectedness**.

```{r table: linear mixed model all_na, echo=FALSE}
...
kable(
  ...
  col.names = c("Question", "Timeline Pair", "Estimate", "SE", "DF", "t-value", "p-value")
) %>%
  kable_styling(
    bootstrap_options = c("striped", "condensed"),
    position          = "left"
...
Mean response on each **Recent Wellbeing** section at three timepoints, where 5 = Strongly Agree and 1 = Strongly Disagree
